...
G  = ct.tf([5,-25], np.convolve([1,2],[1,-2]))
Ts = 1
...
pnd = np.array([-5])       # Polo não dominante

# Polos em z
P = np.exp(np.concatenate((pd,pnd)))
...
Pobs = np.array([Ts/4, Ts/4.1])
Ke   = ct.place(sysz.A.T, sysz.C.T, Pobs) # Proj. Observador

# Defina as condições iniciais dos estados
x0sys = np.array([0, 0]) # Cond. inicial do sistema
x0obs = np.array([0, 0]) # Cond. inicial do observador
