...
def latexTable(t, y):
    """
    Gerar o texto em LaTeX dos critérios de desempenho de várias respostas. O tempo de simulação deve ser o mesmo nos 3 casos.

    Parâmetros
    ----------
    t : array, np.array
        Vetor com o tempo da(s) simulação(ões)
    y : array, np.array, lista de arrays
        Resposta(s) do(s) sistema(s).

    Retorna
    -------
    table : string
        Tabela com os critérios de desempenho escrita em LaTeX.

    Notas
    -----
    Por padrão, o caption da tabela é 'Critérios de desempenho dos controladores' e o label 'tab:CritDesempenho'.

    Faça print(table) e copie o texto do prompt.
    """
    y = np.asarray(y)
    n, *m = y.shape # forma inicial de y
    # Se y for um vetor e não uma matriz, transforma-o numa matriz linha
    ...
    for i, x in enumerate(y):
        crit[i] = stepinfo(t, x)[0:4]

    crit = np.round(crit, 3)

    amb = '{table}'
    cen = '\\centering'
    cap = '\\caption{Critérios de desempenho dos controladores}'
    lab = '\\label{tab:CritDesempenho}'
    tab = '{tabular}'
    c   = '{'+(n+1)*'c'+'}'

    sep = np.concatenate(((n-1)*['&'], ['\\\\']))
    ...
    for i, x in enumerate(crit):
        head = head + f'Controlador {i+1} {sep[i]} '
        ts   = ts   + f'{x[0]} {sep[i]} '
        OS   = OS   + f'{x[1]} {sep[i]} '
        US   = US   + f'{x[2]} {sep[i]} '
        tr   = tr   + f'{x[3]} {sep[i]} '

    table = f'\\begin{amb}[ht]\n{cen}\n{cap}\n{lab}\n\\begin{tab}' \
           +f'{c}\n\\toprule\n{head}\n\midrule\n{ts}\n{OS}\n{US}' \
           +f'\n{tr}\n\\bottomrule\n\\end{tab}\n\\end{amb}'
...
def simulaSmith(Gp, Gn, L, Cz):
    ...
    x0 = np.zeros(len(Gp.den[0][0])-1)
    _n, _m = len(Cz.num[0][0])-1, len(Cz.den[0][0])-1
    _u, _e = np.zeros(_m), np.zeros(_n)

    _uh = np.zeros(len(Gn.num[0][0])-1)
    xh0 = np.zeros(len(Gn.den[0][0])-1)


    for k in K[1:]:
        ek = r[k] - y[k] - yh[k] + yh[k-nL]
        uk = update_uk(Cz, ek, _u, _e)

        _u = np.concatenate(([uk], _u[0:-1])) # Atualiza uk, uk-1,
        _e = np.concatenate(([ek], _e[0:-1])) # Atualiza ek, ek-1
        u[k] = uk # Salva uk

        _, yk, xk   = sim_cont(Gp, t0, Ts, qnt, uk, x0)
        _, yhk, xhk = sim_cont(Gn, t0, Ts, qnt, uk, xh0)
